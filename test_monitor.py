...
def main():
    ...
    payload = "{\"spu_ext_id\":191 }"

    while True:
        xsign, timestamp = generate_x_sign()
        # res2 = requests.post("http://localhost:8000/sign", json={"path": "/shop/v1/box/box_set/extract", "body": payload, "contextId": context_id })
        # res2Json = res2.json()
        # print(res2Json)
        ...
        headers = {
            ...
            # 'tz': 'America/Edmonton',
            'tz': timezone,
            'Td-Session-Path': '/shop/v1/box/box_set/extract',
            'Country': 'CA',
        ...
        response = requests.post('https://prod-na-api.popmart.com/shop/v1/box/box_set/extract', json=req_payload,
                             headers=headers)
        print(response.text)
        time.sleep(5)
        # print(response.text)
